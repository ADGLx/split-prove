...
const IMAGE_OVERRIDE_ENV_VARS = ['MIDNIGHT_NODE_IMAGE', 'MIDNIGHT_INDEXER_IMAGE', 'MIDNIGHT_PROOF_SERVER_IMAGE'];
...
export const freshStart = async (config: Config, logger: Logger): Promise<StartedDockerComposeEnvironment> => {
  ...
  const hasImageOverride = IMAGE_OVERRIDE_ENV_VARS.some((envVar) => process.env[envVar]);
  const pullArgs = ['compose', '-f', 'standalone.yml', 'pull'];
  if (hasImageOverride) {
    pullArgs.push('--ignore-pull-failures');
    logger.info('Pulling images; local image overrides are allowed to skip registry pulls...');
  } else {
