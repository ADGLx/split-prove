...
`git diff 641d18e5...HEAD` (merge-base based): split-prove commits plus local working-tree changes sit on top of the `no-sk` baseline. The latest working-tree delta keeps split admission ledger-verified by checking each split bundle's root for membership in the configured registry contract's historic-roots set (`split_registry_root_history` walks the `HistoricMerkleTree`'s roots map — the same membership the in-circuit `checkRoot` enforces). The `wallet_registry` contract is deployed by the toolkit at genesis (`GenesisGenerator::deploy_wallet_registry`), so `LedgerParameters.split_registry_contract` resolves to a real on-chain contract from block 0. The wallet's first spend reads the contract's current registry tree — the height-20 tree stored at slot 0 of its `HistoricMerkleTree<20, Bytes<32>>`, which admission deserializes as a `BoundedMerkleTree<20>` — via `wallet_registry_call::build_chain_registration_witness` and emits the real `registryRoot` (the current root, itself a member of the historic set). The `MIDNIGHT_SPLIT_REGISTRY_DEV_ACCEPT_ALL=1` bypass is no longer set by `make native-node` / `make native-indexer` — the chain checks every split-spend bundle's root against the historic-roots set. The bypass code remains in the ledger as a diagnostic knob, and a complementary `MIDNIGHT_LOCAL_FORCE_CORRUPT_REGISTRY_WITNESS=1` knob in the proof-server's `prove_local_wallet_split_spend` swaps in a corrupted-synthetic witness (a root the chain never held) for negative-control testing (admission rejects with `Custom(139)`; all four registry-rejection variants, incl. `MalformedTransaction::SplitRegistryRootNotRecognized`, collapse to `UnknownError`).
...
- The rebuilt node validates `registry_root` against `LedgerParameters.split_registry_contract` by testing membership in the registry contract's historic-roots set, then verifies both proofs during zswap `well_formed_with_registry_policy()` checks. Synthetic local roots (never registered on chain) require `MIDNIGHT_SPLIT_REGISTRY_DEV_ACCEPT_ALL=1`.
