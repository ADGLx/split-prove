...
Use local-dev option 6 (**Prepare split-prove e2e funding**) before running the e2e. It funds the split-prove wallet's shielded output **and** runs `make register-wallet` to put the wallet's `reg_leaf` on-chain. Both funding and registration are on-chain state, so both must be redone on every fresh chain — i.e. after each `make local-nodes` restart. (Set `MIDNIGHT_SPLIT_PROVE_SKIP_REGISTER=1` to fund without registering and run `make register-wallet` yourself.) The demo then scans for the funded shielded output, builds the client proof, asks `/v2/prove-split-spend` for the server proof, assembles the transaction, balances Dust, submits it to the rebuilt local node, and waits for inclusion.

The full registry-anchoring loop now runs end-to-end. The `wallet_registry` contract is deployed at genesis (`GenesisGenerator::deploy_wallet_registry`), the wallet submits `register(reg_leaf)` once per chain via `make register-wallet` (or via option 6 above), and every subsequent split spend reads the live contract state to build its Merkle path. Admission accepts `registryRoot` if it is a member of the contract's historic-roots set (the same membership the in-circuit `checkRoot` enforces), so a proof built against root `R` stays valid even after later registrations advance the current root past `R`. The legacy `MIDNIGHT_SPLIT_REGISTRY_DEV_ACCEPT_ALL=1` bypass is no longer set by `make native-node` / `make native-indexer` and the supported path is real-root validation. The bypass code remains in the ledger as a diagnostic switch.
